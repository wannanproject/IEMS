var saveAddWindows = function () {
    var materialNo = App.textAddMaterialNo.getValue();
    //var palletId2 = App.txtAddPalletNo2.getValue();
    var materialName = App.textAddMaterialName.getValue();
    ...
    if (materialNo == null || materialNo == "") {
        Ext.Msg.alert("错误", '请输入物料编号!');
        return;
    }
    App.direct.AddMaterialInfo(materialNo, materialName,  {
        success: function (result) {
            if (result == "") {
                //Ext.Msg.alert('成功', materialNo + materialName);
                Ext.Msg.alert('成功', "物料添加成功！");
            }
            else {
...
var showAddWindow = function () {
    App.textAddMaterialNo.setValue("");
    App.textAddMaterialName.setValue("");
